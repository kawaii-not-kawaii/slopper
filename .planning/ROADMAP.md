...
- [ ] **Phase 8: AGP-9 — Atomic Build-Logic Migration + compileSdk 36** (plan 08.1 in progress — HALTED at Task-5 smoke on blocker B-08-02) - The indivisible core: Gradle 9.4.1 activation (commit 1) + AGP 9.2.1 + built-in Kotlin + `CommonExtension` fix + `compilerOptions` + Hilt 2.59.2 + compileSdk 36, build green across all ~14 modules. **STATUS:** commits 1–6 + baselineprofile 1.5.0-alpha06 (AR-08-01) landed & bisectable (last-good `631b2b0`); blocked by B-08-02 — KSP 2.2.20-2.0.4 × AGP-9 built-in Kotlin incompatibility, every fix crosses a locked policy line (D-04b / D-09). Awaiting planner/user decision (recommended: minimal Kotlin 2.2.21 + KSP 2.2.21-2.0.5 bump). See `08-01-SUMMARY.md` + STATE.md B-08-02.
