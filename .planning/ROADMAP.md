...
- [x] **Phase 10: CI-SIGNING — Isolated Assemble/Signing Probe** (completed 2026-05-31) - Non-gating `assembleDebug` probe added to both CI workflows (Forgejo + GitHub) as `continue-on-error`. **Local result: BUILD SUCCESSFUL** — the EdEC/BouncyCastle blocker appears resolved under AGP 9 (assembleDebug passes locally). CI runners will confirm. If green on CI, the compile-only gate can be promoted to a real assemble/signing gate. Also fixed: missing `consumer-rules.pro` files in 14 library modules (AGP 9 now enforces their existence). Forgejo CI updated to agp9 cache key + parity with GitHub CI (wrapper validation, concurrency, --no-daemon, test report upload).
...
| 10. CI-SIGNING | v1.1 | direct | Complete | 2026-05-31 |
