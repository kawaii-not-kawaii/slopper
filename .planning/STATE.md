...
**Phase 08 plan 08.1 is HALTED AGAIN at the Task-5 smoke** — B-08-01 is resolved (baselineprofile 1.5.0-alpha06, `631b2b0`), but a NEW blocker **B-08-02** surfaced: KSP 2.2.20-2.0.4 is incompatible with AGP-9 built-in Kotlin, and every fix crosses a locked policy line (D-04b no-Kotlin/KSP-bump, or D-09 no-crutch-flag). Commits `888c65a`→`631b2b0` are in place and clean (last-good `631b2b0`). **A planner/user policy decision on B-08-02 is required before resuming.** Recommended: lift D-04b for the minimal Kotlin 2.2.21 + KSP 2.2.21-2.0.5 patch bump, then resume the smoke (`./gradlew --stop && ./gradlew :core:common:compileDebugKotlin`), and continue with the remaining commits (kotlinOptions→compilerOptions, compileSdk 36, CI cache-key agp8→agp9 + guards) and the Task-10 full gate. The remaining DSL edits (Tasks 6/8/9) are unaffected by B-08-02 and ready to apply once configuration is unblocked.
