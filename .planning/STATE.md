...
- **Kotlin/KSP: stay on stable 2.2.20 / 2.2.20-2.0.4 (FINAL, B-08-03 resolved 2026-05-31)** — AGP-9 built-in Kotlin would require KSP2 ⇒ Kotlin 2.3.x (rejected — destabilizes Apollo/Hilt/Compose/detekt). Instead AGP-9 runs with **`android.builtInKotlin=false`** + the traditional `kotlin("android")` application (KSP1 path, KSP-team-prescribed). The 2.2.21 bump (`1b17a80`) is reverted back to the locked-matrix 2.2.20 / 2.2.20-2.0.4. compose-compiler plugin auto-tracks Kotlin via `version.ref`.
- **D-BIK: AGP-9 built-in Kotlin DEFERRED (B-08-03 resolution)** — this milestone lands AGP 9 with `android.builtInKotlin=false` and explicit `kotlin("android")` application. Built-in Kotlin (AGP9-02 success-criterion #2) is deferred until KSP2 + Kotlin 2.3.x are safe for Apollo 5 / Hilt 2.59 / detekt. `android.builtInKotlin=false` is an INTENTIONAL, supported AGP-9 setting here — the no-crutch guard must allow it (only `enableLegacyVariantApi`/`newDsl` remain forbidden).
...
**Phase 08 plan 08.1 HALTED at Task-5 smoke — B-08-04 ACTIVE (architectural decision required).** The B-08-03 resolution (option b: `android.builtInKotlin=false` + traditional `kotlin("android")` on Kotlin 2.2.x) was executed in full (Commits A `35748ff` + B `cbf9689`) but is **empirically false**: the KGP 2.2.x `kotlin.android` plugin crashes at apply time under AGP 9 with `ApplicationExtensionImpl cannot be cast to BaseExtension` (AGP 9 removed `BaseExtension`). Verified on both 2.2.20 and 2.2.21; the entire 2.2.x line has NO AGP-9-compatible config. See B-08-04 (active) for the full diagnosis + closed decision space. **Decision needed:** (a) lift D-04b → adopt Kotlin 2.3.x + KSP 2.3.x + AGP-9 built-in Kotlin (only green path; re-verify Apollo/detekt; #8865 likely needs §Pitfall-6 fallback); (b) defer AGP-9 entirely (roll back to last-good); (c) drop Hilt/Apollo KSP (not viable). **Recommendation: (a).** Commits A+B are kept as bisectable provenance and will be reverted if (a) is chosen. **Active goal: fix all Dependabot debt properly — but the AGP-9 landing is now gated on resolving the Kotlin-2.3.x-vs-D-04b conflict.**
