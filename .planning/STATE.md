...
**Phase 08 plan 08.1 RESUMING — B-08-03 resolved (option b: `android.builtInKotlin=false`).** AGP-9 lands with the traditional KSP1/Kotlin-2.2.x application; built-in Kotlin deferred (D-BIK). Resume steps: (1) revert Kotlin/KSP 2.2.21 → 2.2.20 / 2.2.20-2.0.4; (2) add `android.builtInKotlin=false` to `gradle.properties`; (3) restore `kotlin("android")` application at the 5 sites dropped in `fa25942` (2 convention plugins via `apply("org.jetbrains.kotlin.android")`, root `alias(...) apply false`, baselineprofile `alias(...)`, catalog `kotlin-android` alias) — KEEP the bare-CommonExtension + AGP-9 DSL fixes and the KotlinAndroidProjectExtension block; (4) re-run the `:core:common:compileDebugKotlin` smoke (expect green); (5) Task 6 baselineprofile kotlinOptions→compilerOptions; (6) Task 8 compileSdk 36 ×2; (7) Task 9 CI cache-key agp8→agp9 + guards (targetSdk=35 ×2, compileSdk=36 ×2, forbidden-flags = enableLegacyVariantApi/newDsl ONLY — `builtInKotlin=false` is allowed/expected); (8) Task 10 full gate `compileDebugSources detekt ktlintCheck test` (incl. empirical detekt-1.23.8 on Gradle-9). **Active goal: fix all Dependabot debt properly — this lands AGP/Gradle/Hilt/benchmark/compileSdk; Phase 9 then does Media3/nextlib/activity/core; prune the dependabot.yml ignore list afterward.**
