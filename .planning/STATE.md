...
last_updated: "2026-05-31T15:10:00.000Z"
progress:
  total_phases: 4
  completed_phases: 1
  total_plans: 3
...
- **Kotlin/KSP: 2.2.21 bump APPLIED but EMPIRICALLY INSUFFICIENT (B-08-03, 2026-05-31)** — the user-approved B-08-02 fix (Kotlin 2.2.20→**2.2.21** + KSP 2.2.20-2.0.4→**2.2.21-2.0.5**) was applied + committed (`1b17a80`), but the choke-point smoke STILL fails with the identical `KSP is not compatible with AGP built-in Kotlin` error. Root cause: **every KSP on the 2.2.x line (incl. `2.2.21-2.0.5`) is the unified KSP1 plugin, which rejects AGP-9 built-in Kotlin at apply-time** (`KspSubplugin.kt:678`); `ksp.useKSP2=true` does NOT bypass it. AGP-9 built-in Kotlin requires a **KSP2-only** plugin = **KSP ≥ 2.3.0** (new bare scheme) = **Kotlin 2.3.x**. The "Still NO 2.3.x" (D-04b) ceiling is therefore the active blocker — see B-08-03 below. compose-compiler plugin auto-tracks Kotlin via `version.ref` (will follow whatever Kotlin lands).
...
**Phase 08 plan 08.1 HALTED at B-08-03 — the B-08-02 fix was applied but proved empirically insufficient; a NEW locked-policy decision is required.** The Kotlin/KSP 2.2.21 bump (`1b17a80`) is committed and correct, but the AGP-9 × KSP built-in-Kotlin collision **cannot** be cleared on the Kotlin 2.2.x line — it requires a KSP2-only plugin (KSP ≥ 2.3.0) which is locked to **Kotlin 2.3.x**, hitting the **D-04b "Still NO 2.3.x" hard ceiling**. `ksp.useKSP2=true` was tried and does not bypass it. **AWAITING USER DECISION (see Blockers/B-08-03):** (a) RECOMMENDED — lift the D-04b 2.3.x ceiling and adopt Kotlin 2.3.x + KSP 2.3.x bare (re-verify Hilt/Compose/Apollo/detekt on 2.3.x; detekt 1.23.8 will likely need its §Pitfall-6 fallback); (b) accept the `android.builtInKotlin=false`+`kotlin("android")` D-09/D-06 crutch (not recommended); (c) defer AGP-9. **Last-good commit: `1b17a80`** (all prior work bisectable; nothing to revert). Once the decision is made, resume from Task 5 (re-run the `:core:common:compileDebugKotlin` smoke), then Tasks 6 (kotlinOptions→compilerOptions), 8 (compileSdk 36 ×2), 9 (CI cache-key agp8→agp9 + targetSdk×2/no-crutch/compileSdk guards), 10 (full gate). **Active goal unchanged: fix all Dependabot debt properly — landing this toolchain migration unblocks the deferred AGP/Gradle/Kotlin/KSP/Hilt/benchmark/compileSdk deps in `.github/dependabot.yml`'s ignore list.**
