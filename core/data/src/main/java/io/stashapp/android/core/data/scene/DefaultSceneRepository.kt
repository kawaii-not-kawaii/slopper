...
@Singleton
class DefaultSceneRepository @Inject constructor(
    private val apollo: ApolloClient,
    private val endpointProvider: StashEndpointProvider,
) : SceneRepository {

    override fun pagedScenes(query: SceneQuery): Flow<PagingData<SceneSummary>> = Pager(
        config = PagingConfig(
            pageSize = 40,
    ...
    override suspend fun scene(id: String): AppResult<SceneDetail> {
        ...
        return try {
            ...
            if (response.hasErrors()) {
                ...
                val scene = response.data?.findScene
    ...
    override suspend fun addPlay(sceneId: String): AppResult<Unit> = mutate {
        apollo.mutation(SceneAddPlayMutation(id = sceneId)).execute()
    }

    override suspend fun incrementO(sceneId: String): AppResult<Int> = mutateMapped {
        val resp = apollo.mutation(SceneIncrementOMutation(id = sceneId)).execute()
        resp to resp.data?.sceneAddO?.count
    }

    override suspend fun decrementO(sceneId: String): AppResult<Int> = mutateMapped {
        val resp = apollo.mutation(SceneDecrementOMutation(id = sceneId)).execute()
        resp to resp.data?.sceneDeleteO?.count
    }

    override suspend fun setRating(sceneId: String, rating100: Int?): AppResult<Unit> = mutate {
        apollo.mutation(
            SceneUpdateMutation(
    ...
    override suspend fun setOrganized(sceneId: String, organized: Boolean): AppResult<Unit> = mutate {
        apollo.mutation(
            SceneUpdateMutation(
                input = SceneUpdateInput(
                    id = sceneId,
    ...
    private suspend inline fun <T> mutateMapped(
        ...
        when {
            resp.hasErrors() -> AppResult.Failure(
                AppError.Server(resp.errors?.joinToString { it.message } ?: "Mutation failed"),
            )
